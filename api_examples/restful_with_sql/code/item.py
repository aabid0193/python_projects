...
class Item(Resource):
    ...
    def get(self, name):
        ...
        if item:
            return item
        return {'message': 'Item not found'}, 404  #else statement implicit
    ...
    def find_by_name(cls, name):
        ...
        row = result.fetchone() # One or no rows coming back
        connection.close()
    ...
    def post(self, name):
        ...
        if Item.find_by_name(name): # you can also call with self.find_by_name
    ...
    def delete(self, name):
        ...
        connection = sqlite3.connect('data.db')
        cursor = connection.cursor()
        query = "DELETE FROM items WHERE name = ?"  #forgetting WHERE clause will delete everything
    ...
    def update(cls, item):
        connection = sqlite3.connect('data.db')
        cursor = connection.cursor()
        query = "UPDATE items SET price = ?"  #forgetting WHERE clause will delete everything
